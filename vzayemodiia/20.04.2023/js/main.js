...
async function LogRequest(dataLog, data) {
  ...
  if (dataLog.code === 200) {
    if (data.alchogol !== undefined) {
      const count =
        data.alcohol.length +
        data.caffeine.length +
        data.food.length +
...
document.addEventListener('click', event => {
  ...
  if (dataInsturctions_drug1) {
    preparedInstructions.push(...prepareInstruction(dataInsturctions_drug1, appData.drug_1));
  }

  if (dataInsturctions_drug2) {
    preparedInstructions.push(
      ...prepareInstruction(dataInsturctions_drug2, appData.drug_1)
    );
  }

  function prepareInstruction(originalData, name) {
...
async function translateAnswerInstructions(
  ...
  const originalLang = SETTING.defaultLanguage;
  ...
  if (texts.length) {
    ...
    const options = {
      ...
      body: {
        q: texts,
        target:
          appData.language === 'original' ? originalLang : appData.language,
      },
      showModal: true,
      logRequest: true,
      timeout: SETTING.api.timeoutTranslation,
  ...
  Object.keys(SETTING.language).forEach((lang) => {
    window.console.log(originalLang);
    actions.push({
