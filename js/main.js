...
function showModal(textHeader) {
  let textBody = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  let modalActions = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
  if (!textHeader) {
    return false;
  }
...
function showModalInstruction(textHeader) {
  let textBody = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  if (!textHeader) {
    return false;
  }
  ...
  document.body.appendChild(modal);
  document.body.classList.add('modal-open');
  document.documentElement.style.setProperty('--modal-height', '100%');
  const modalHeight = document.querySelector('.modal--instruction').offsetHeight;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchWithTimeout(resource, options) {
    var _SETTING$api$timeout;
    const {
      timeout = (_SETTING$api$timeout = SETTING.api.timeout) !== null && _SETTING$api$timeout !== void 0 ? _SETTING$api$timeout : 15000
    } = options;
  ...
  async function Request(url) {
    ...
    if (response.status === 200) {
      ...
      data = (_await$response$json = await response.json()) !== null && _await$response$json !== void 0 ? _await$response$json : {};
    }
    ...
    if (response.status != 200 && response.status != 404 && options.showModal != false) {
  ...
  async function LogRequest(dataLog, data) {
    ...
    if (dataLog.code === 200) {
      if (data.alcohol) {
        ...
        body.message = 'data received';
      }
    } else if (dataLog.code === 404) {
  ...
  function updateHeader() {
    const isInput1Valid = document.querySelector('input[name=drug_1]').checkValidity();
    const isInput2Valid = document.querySelector('input[name=drug_2]').checkValidity();
    if (appData.drug_1.trim().length > 0 && isInput1Valid && isInput2Valid) {
    ...
    if (appData.drug_1 && appData.drug_2 && appData.isFetched && appData.isFetchedDrug2) {
    ...
    if (EXAMPLE.idx > 0 && !appData.isLoading) {
      document.querySelectorAll('[data-example] > [data-show]').forEach(title => {
        if (appData.isFetched && title.getAttribute('data-show') == 'another_example') {
          title.classList.remove('hidden');
        } else if (!appData.isFetched && title.getAttribute('data-show') == 'default') {
          title.classList.remove('hidden');
        } else {
  ...
  function updateBody() {
    if (appData.isFetched && (appData.drugs.length || appData.alcohol.length || appData.food.length || appData.caffeine.length || appData.instructions.length)) {
    ...
    const instructions_tab = document.querySelector('[data-target="tab-instructions"]');
    ...
    document.querySelectorAll('[id^="tab-"]').forEach(tab => {
      ...
      if (appData.isLoading) {
        ...
        titles.forEach(title => {
          if (id == 'loading') {
            if (appData.isTranslating && title.getAttribute('data-show') == 'translating') {
              title.classList.remove('hidden');
            } else if (!appData.isTranslating && title.getAttribute('data-show') == 'default') {
              title.classList.remove('hidden');
            } else {
              title.classList.add('hidden');
            }
      ...
      if (appData.isFetched) {
        if (appData[id] && appData[id].length > 0) {
          ...
          if (!appData.isArticlesRendered[id]) {
            ...
            if (id === 'instructions') {
              appData.instructions.forEach(info => {
                const instruction = document.getElementById(`instruction-${info.title}`);
          ...
          titles.forEach(title => {
            if (id == 'drugs') {
              if (!appData.isFetchedDrug2 && title.getAttribute('data-show') == 'input-drug2') {
                title.classList.remove('hidden');
              } else if (appData.isFetchedDrug2 && title.getAttribute('data-show') == 'not-found') {
                title.classList.remove('hidden');
              } else {
                title.classList.add('hidden');
              }
  ...
  `;
    ...
    if (data.text.length > 150) {
      more = `<span class="article__more">${SETTING.language[appData.language]['show_more']}</span>`;
      moreClass = '';
    }
    return `
  ...
  const rightArrowIcon = document.querySelector('.scrollable-tabs-container__indicator--right svg');
  const rightArrow = document.querySelector('.scrollable-tabs-container__indicator--right');
  const leftArrowIcon = document.querySelector('.scrollable-tabs-container__indicator--left svg');
  const leftArrow = document.querySelector('.scrollable-tabs-container__indicator--left');
  ...
  function manageIcons() {
    ...
    const maxScrollValue = tabsContainer.scrollWidth - tabsContainer.clientWidth - 20;
  ...
  document.addEventListener('click', event => {
    ...
    if (!tab || tab.classList.contains('active') || document.body.classList.contains('tour') && !document.body.classList.contains('tour_tabs_enable')) {
      return false;
    }
    tab.scrollIntoView({
      behavior: 'smooth',
  ...
  document.addEventListener('click', event => {
    ...
    if (EXAMPLE.idx in SETTING.examples) {
      ...
      EXAMPLE.drug_1 = (_drugs$ = drugs[0]) !== null && _drugs$ !== void 0 ? _drugs$ : '';
      EXAMPLE.drug_2 = (_drugs$2 = drugs[1]) !== null && _drugs$2 !== void 0 ? _drugs$2 : '';
      EXAMPLE.idx++;
  ...
  appData.nodeForm.addEventListener('submit', async event => {
    event.preventDefault();
    appProxy.drug_1 = appData.drug_1.trim();
    appProxy.nodeDrug_1.value = appData.nodeDrug_1.value.trim();
    let combination = appData.drug_1;
    if (appData.drug_2.length > 0) {
      appProxy.drug_2 = appData.drug_2.trim();
      appProxy.nodeDrug_2.value = appData.nodeDrug_2.value.trim();
      if (appData.drug_2.length) {
        combination += ',' + appData.drug_2;
      }
    }
    if (combination.length <= 1 || appData.lastFetchedCombination == combination) {
      return false;
    }
    ...
    if (appData.drug_2.trim().length > 0) {
      appProxy.isFetchedDrug2 = true;
    } else {
      appProxy.isFetchedDrug2 = false;
    }
    const url = `${SETTING.api.compatibility}?DrugName=${encodeURIComponent(combination)}`;
    const urlInstructions = SETTING.api.instructions;
    const options = {
      method: 'GET',
      headers: {
    ...
    let dataInsturctions_drug1 = await Request(urlInstructions, optionsInstructions_drug1);
    let dataInsturctions_drug2;
    if (appData.drug_2) {
      dataInsturctions_drug2 = await Request(urlInstructions, optionsInstructions_drug2);
    }
    ...
    function prepareInstruction(originalData) {
      ...
      drugTypes.forEach(drug => {
        const drugNameUk = drug.attributes.product_brand_series_name_uk || drug.attributes.product_brand_series_name;
        ...
        const drugInstructionId = Array.isArray(drug.relationships['product-instruction'].data) ? drug.relationships['product-instruction'].data[0].id : drug.relationships['product-instruction'].data.id;
        ...
        const drugInstructionDefault = fullInstruction.attributes.product_instruction_description_uk || fullInstruction.attributes.product_instruction_description;
        const drugInstructionUk = fullInstruction.attributes.product_instruction_description_uk || fullInstruction.attributes.product_instruction_description;
        const drugInstructionRu = fullInstruction.attributes.product_instruction_description_ru;
        const drugInstructions = [drugInstructionDefault, drugInstructionUk, drugInstructionRu];
        const hasInstruction = drugInstructions.some(instruction => instruction);
        if (!hasInstruction) {
          return;
        }
        prepInst.push({
    ...
    window.console.log(isActiveTabFound);
    ['drugs', 'alcohol', 'food', 'caffeine', 'instructions'].forEach(key => {
      window.console.log(appData.activeTab);
      if (isActiveTabFound === false && appData[key].length > 0) {
    ...
    if (isActiveTabFound === false && appData.activeTab === 'tab-instructions') {
      window.console.log('inside');
      isActiveTabFound = true;
      appProxy.activeTab = 'tab-drugs';
      document.getElementById('tab-drugs').classList.remove('hidden');
      document.getElementById('tab-instructions').classList.add('hidden');
      appProxy.nodeActiveTabSection = document.getElementById('tab-drugs');
      document.querySelector('[data-target^="tab-drugs"]').classList.add('active');
      document.querySelector('[data-target^="tab-drugs"]').scrollIntoView({
        behavior: 'smooth',
        block: 'nearest',
        inline: 'center'
      });
      document.querySelector('[data-target^="tab-instructions"]').classList.remove('active');
    ...
    // window.console.log({activeTab: appData.activeTab});
  ...
  async function translateAnswer() {
    let originalData = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    let withoutReloadingPage = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    const data = structuredClone(originalData);
    ...
    Object.keys(data).forEach(key => {
      ...
      data[key].forEach(answer => {
        if (appData.language != 'original' && answer.language != appData.language && answer.text.length) {
          count[key] += 1;
    ...
    if (texts.length) {
      var _dataTranslate$respon, _dataTranslate$respon2;
      ...
      const delay = withoutReloadingPage ? SETTING.api.delayTranslation || 2000 : null;
      const dataTranslate = await Request(url, options, delay);
      let matchedTranslationsCount = 0;
      if ((dataTranslate === null || dataTranslate === void 0 ? void 0 : dataTranslate.responseStatus) == 'success' && dataTranslate !== null && dataTranslate !== void 0 && (_dataTranslate$respon = dataTranslate.responseMessage) !== null && _dataTranslate$respon !== void 0 && (_dataTranslate$respon2 = _dataTranslate$respon.translations) !== null && _dataTranslate$respon2 !== void 0 && _dataTranslate$respon2.length) {
        Object.keys(data).forEach(key => {
          if (count[key] > 0) {
            for (let i = 0; i < count[key]; i++) {
              data[key][i]['text'] = dataTranslate.responseMessage.translations[i + matchedTranslationsCount].translatedText;
            }
    ...
    Object.keys(data).forEach(key => {
      data[key].forEach(answer => {
        if (appData.language == 'original' || answer.language == appData.language) {
  ...
  async function translateAnswerInstructions() {
    ...
    // const originalLang = GET_PARAM('language');
    const originalLang = 'uk';
    const data = structuredClone(originalData);
    if (withoutReloadingPage) {
      appProxy.isFetched = false;
      appProxy.isLoading = true;
    ...
    data.forEach(answer => {
      if (!answer[appData.language].description) {
        count += 1;
        const ref = answer.uk.description ? answer.uk.description : answer.ru.description;
        texts.push(ref);
      }
    });
    if (texts.length) {
      var _dataTranslate$respon3, _dataTranslate$respon4;
      const url = SETTING.api.translation;
      const options = {
        method: 'POST',
        headers: {
        ...
        body: {
          q: texts,
          target: appData.language === 'original' ? originalLang : appData.language
        },
        showModal: true,
        logRequest: true,
        timeout: SETTING.api.timeoutTranslation
      ...
      const dataTranslate = await Request(url, options, delay);
      let matchedTranslationsCount = 0;
      if ((dataTranslate === null || dataTranslate === void 0 ? void 0 : dataTranslate.responseStatus) == 'success' && dataTranslate !== null && dataTranslate !== void 0 && (_dataTranslate$respon3 = dataTranslate.responseMessage) !== null && _dataTranslate$respon3 !== void 0 && (_dataTranslate$respon4 = _dataTranslate$respon3.translations) !== null && _dataTranslate$respon4 !== void 0 && _dataTranslate$respon4.length) {
        if (count > 0) {
          for (let i = 0; i < count; i++) {
            data[i][appData.language].description = dataTranslate.responseMessage.translations[i + matchedTranslationsCount].translatedText;
          }
    ...
    if (texts.length) {
      var _dataTranslate$respon5, _dataTranslate$respon6;
      const url = SETTING.api.translation;
      const options = {
        method: 'POST',
        headers: {
      ...
      const delay = withoutReloadingPage ? 1000 : null;
      const dataTranslate = await Request(url, options, delay);
      console.log(withoutReloadingPage);
      console.log(delay);
      let matchedTranslationsCount = 0;
      if ((dataTranslate === null || dataTranslate === void 0 ? void 0 : dataTranslate.responseStatus) == 'success' && dataTranslate !== null && dataTranslate !== void 0 && (_dataTranslate$respon5 = dataTranslate.responseMessage) !== null && _dataTranslate$respon5 !== void 0 && (_dataTranslate$respon6 = _dataTranslate$respon5.translations) !== null && _dataTranslate$respon6 !== void 0 && _dataTranslate$respon6.length) {
        if (count > 0) {
          for (let i = 0; i < count; i++) {
            data[i].original.title = dataTranslate.responseMessage.translations[i + matchedTranslationsCount].translatedText;
          }
          matchedTranslationsCount += count;
        }
      }
    }
    const translated = data.map(instruction => {
      if (instruction.original.title && instruction[appData.language].description) {
        return {
  ...
  document.addEventListener('click', event => {
    ...
    Object.keys(SETTING.language).forEach(lang => {
      actions.push({
        name: SETTING.language[originalLang][lang],
    ...
    showModal(SETTING.language[originalLang]['select_lang'], '', actions);
  });
  document.querySelectorAll('[data-lang]').forEach(item => {
    var _SETTING$language$app;
    const key = item.getAttribute('data-lang');
    const translation = (_SETTING$language$app = SETTING.language[appData.language][key]) !== null && _SETTING$language$app !== void 0 ? _SETTING$language$app : null;
    if (!key && !translation) {
      return false;
    }
